...
class ClusteringDataset1_3(Dataset):
    """
    返り値（1サンプル）:
      {
    ...
    def __getitem__(self, idx: int) -> Dict[str, object]:
        ...
        cache_dir = os.path.join(self.root_dir, doc_id, "1_3_cache")
        ...
        if os.path.exists(cache_path):
            ...
            torch.save(final_text_mask, cache_path)
...
train_dataset = ClusteringDataset1_3(
    root_dir=root_dir,
    canvas_width=2048,
    patch_size=256,
    test_mode=False,
...
test_dataset = ClusteringDataset1_3(
    root_dir=root_dir,
    canvas_width=2048,
    patch_size=256,
    test_mode=True,
...
for epoch in range(start_epoch, num_epochs + 1):
    ...
    for batch in train_loader:
        ...
        mask = batch["final_text_mask"].to(device, non_blocking=True).unsqueeze(1)  # (B,1,H,W)
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            mask = batch["final_text_mask"].to(device, non_blocking=True).unsqueeze(1)
